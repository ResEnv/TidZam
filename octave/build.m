...
dataset = struct ();
dataset = setfield (dataset, "name", classe);
dataset = setfield (dataset, "yes", cl1);
dataset = setfield (dataset, "no", cl2);
dataset = setfield (dataset, "size", size_data);

if(DEBUG)
printf("\nClasse +: %d samples\nClasse -: %d samples\nData size: %dx%d\n\n", size(cl1)(1), size(cl2)(1), size_data);
end

save(file_out, '-binary', 'dataset');
